      ...
      {isFetchingPost && isFetchingComments ? (
        ...
        post && (
          <div className="max-w-2xl px-8 py-4 bg-white rounded shadow">
            <h1 className="text-3xl font-bold mb-4 text-center text-blue-600">{post.title}</h1>
            <p className="text-lg mb-4 text-gray-700">Par {post.username}</p>
            <p className="text-lg mb-4 text-gray-600">{post.description}</p>
            <p className="text-sm mb-4 text-gray-500">Dernière mise à jour : {new Date(post.updatedAt).toLocaleDateString()}</p>
            <h2 className="text-2xl font-bold mb-4 text-center text-blue-600">Comments</h2>
            {comments?.map((comment, index) => (
              <div key={index} className="mb-4 p-2 border border-gray-200 rounded">
                <h3 className="text-xl font-bold text-blue-600">{comment.title}</h3>
